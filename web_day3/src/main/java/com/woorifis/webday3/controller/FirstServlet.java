...
public class FirstServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// HTTP의 stateless 속성을 극복해보자!! - scope 들을 사용해보자
		// request - parameter & attribute - forward 까지 전달
		String type = request.getParameter("type");; // 클라이언트에서만 발생, String
		request.setAttribute("reqAttr", "resquest"); // 추가할 수 있는 데이터, 객체 관리
		
		// session - 여러개의 request 동안 유지
		HttpSession session = request.getSession();
		session.setAttribute("sesAttr", "sessiong");
		
		// application - 여러개의 session 동안 유지
